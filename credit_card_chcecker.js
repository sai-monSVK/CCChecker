...
function validateCard(array) {
  ...
  for (let i = array.length-1; i >= 0; i--) {
    if (i % 2 == 0) {
      let interCount = array[i]*2;
      newArray.unshift(interCount);
    } else {
      //console.log(array[i])
    ...
    for (let j = newArray.length -1; j >= 0;j--) {
      if (newArray[j] >= 10) {
        let interCountt = newArray[j] - 9;
        resultArray.unshift(interCountt);
      } else {
        resultArray.unshift(newArray[j]);
      }
    };
    let sum = resultArray.reduce(function(previousValue, currentValue) { return previousValue + currentValue});
    let result;
    //console.log(sum);
    if (sum % 10 === 0){
      result = true;
  ...
  function idInvalidCompanies (array) {
    ...
    for (card of invalidCards) {
      ...
      let uniqueCompanies=[...new Set(invalidCardCompanies)];
